...
- **Dominio propio del cliente** (`app.clienteweb.com`, con un CNAME apuntando a tu infra): más profesional (el cliente no ve tu marca en la URL), pero cada dominio necesita su propio certificado ([TLS handshake](what-happens-when-you-type-a-url.es.md#4-tls-handshake-si-es-https)) — Let's Encrypt automatiza la emisión/renovación, pero es infraestructura extra a mantener y a monitorear.
