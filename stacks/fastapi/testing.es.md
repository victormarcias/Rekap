...
`TestClient` (visto en [Endpoints para microservicios](microservice-endpoints.es.md)) funciona para rutas sync y async porque maneja su propio event loop por dentro, pero cuando toda la app usa fixtures async (como la sesión de DB del punto 2), `AsyncClient` encaja mejor: es awaitable y se integra directo con `pytest-asyncio` sin loops anidados.
...
Reutilizando el mismo helper que usa `/login` en producción (ver [Autenticación en FastAPI](authentication.es.md)) para generar un token válido dentro de una fixture, en vez de loguearse de verdad en cada test.
...
---
Relacionado: [Testing — conceptos generales](../../system-design/testing.es.md), [Endpoints para microservicios](microservice-endpoints.es.md), [Autenticación en FastAPI](authentication.es.md), [Sync vs Async en FastAPI](sync-vs-async.es.md).
