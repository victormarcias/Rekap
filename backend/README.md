# Backend

Decisiones concretas de arquitectura de un sistema backend: qué pieza usar y por qué.

- [x] [API Gateway](api-gateway.md)
- [x] [Arquitectura Kafka](kafka.md)
- [x] [Autenticación y Seguridad — conceptos generales](autenticacion.md)
- [x] [Cache invalidation](cache-invalidation.md)
- [x] [Colas de mensajes: SQS vs RabbitMQ vs AMQP](colas-de-mensajes.md)
- [x] [Controller / Service / Repository](controller-service-repository.md)
- [x] [DDoS Attack](ddos-attack.md)
- [x] [Escalabilidad de CPU y Red](escalabilidad-cpu-red.md)
- [x] [HTTP Methods](http-methods.md)
- [x] [Key-Value Store / DynamoDB](dynamodb-key-value-store.md)
- [x] [Load balancers](load-balancers.md)
- [x] [Monolito vs Microservicios](monolito-vs-microservicios.md)
- [x] [Proveedores de Identidad Gestionados (Cognito, Auth0, Firebase Auth)](proveedores-de-identidad-gestionados.md)
- [x] [Redis](redis.md)
- [x] [Webhooks](webhooks.md)
