...
2. **Stateless**: every request carries all the information needed to process it — the server doesn't store context from the same client's previous requests between calls. See [Scalability](../system-design/quality-attributes.md#scalability) (shared session vs in-memory session — statelessness is what enables it).
...
---
Related: [HTTP Methods](http-methods.md), [Scalability](../system-design/quality-attributes.md#scalability), [API Gateway](api-gateway.md), [CDN](../devops/cdn.md), [GraphQL](graphql.md) (the alternative).
