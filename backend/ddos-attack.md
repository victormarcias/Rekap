...
- **Protocol**: exploits a protocol's behavior instead of bandwidth. The classic is the **SYN flood**: sending thousands of `SYN`s (the first step of the [three-way handshake](../system-design/que-pasa-cuando-escribis-una-url.md#3-conexión-tcp--three-way-handshake)) without ever completing the final `ACK` — the server reserves resources for each "half-open" connection until it exhausts them.
...
---
Related: [CDN](../devops/cdn.md), [API Gateway](api-gateway.md), [What happens when you type a URL](../system-design/que-pasa-cuando-escribis-una-url.md) (TCP handshake), [Elasticity](../system-design/atributos-de-calidad.md#elasticidad), [Security](../security/) (application vulnerabilities, different from a traffic attack).
