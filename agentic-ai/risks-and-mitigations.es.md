...
- **Alucinaciones**: el modelo genera información falsa con alta confianza — no "sabe" que está mal, la produce con el mismo tono seguro que una respuesta correcta. Mitigado (no eliminado) con [RAG](rag.es.md) — darle datos reales en vez de dejar que invente.
...
- **Loops infinitos**: el agente queda atrapado en un ciclo sin converger a una respuesta — no solo un problema de UX, es plata real gastada en cada vuelta (ver [Circuit Breaker como límite de gasto](llm-costs.es.md#evitar-gasto-por-loops-que-no-cortan-solos)).
...
- **Costos excesivos**: consumo descontrolado de tokens/API calls — ver [Costos de LLMs](llm-costs.es.md).
...
- **Supervisión humana**: ver [Human-in-the-Loop](agent-design.es.md#human-in-the-loop-hitl) — validación de decisiones críticas.
...
---
Relacionado: [Agentes vs Workflows](agents-vs-workflows.es.md), [Costos de LLMs](llm-costs.es.md), [RAG](rag.es.md), [Diseño de Agentes](agent-design.es.md), [Privacidad y GDPR](../frontend-react/privacy-and-gdpr.es.md).
