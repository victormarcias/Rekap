...
| **Lambda** | Serverless compute | Run code without managing servers, billed per invocation — AWS's FaaS, see [The IaaS → PaaS → Serverless spectrum](../../devops/vps-vs-cloud-run.md#el-espectro-completo-iaas--paas--serverless). |
...
| **Nginx** | Reverse proxy / web server | Terminates TLS, serves static files, acts as a reverse proxy in front of the app. Already covered in [Nginx as reverse proxy](../../devops/deploy-vps.md#4-nginx-como-reverse-proxy). |
