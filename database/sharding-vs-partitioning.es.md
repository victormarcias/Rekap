...
Dividir una tabla **grande en un mismo servidor/instancia** en sub-tablas más chicas (particiones), transparente para las queries. Esto es partitioning **horizontal** — divide **filas**; ver [Vertical Partitioning](scaling-database.es.md#vertical-partitioning) para la otra dimensión, dividir **columnas**.
...
---
Relacionado: [Normalización](normalization.es.md) (denormalización suele acompañar sharding para evitar cross-shard joins), [NoSQL](nosql.es.md) (muchas bases NoSQL shardean nativamente, ej. Cassandra, MongoDB), [Escalabilidad de Base de Datos](scaling-database.es.md) (vertical partitioning, tabla activa vs histórica, snapshot tables).
