...
- **Concurrencia**: múltiples clientes leyendo/escribiendo al mismo tiempo, sin pisarse — ver [Locks](locks.es.md).
- **Durabilidad**: si el proceso se cae a mitad de una escritura, el dato no queda corrupto — ver [Write-Ahead Log](rdbms.es.md#write-ahead-log-wal--cómo-no-se-pierde-nada-si-el-servidor-se-cae).
- **Consultas eficientes**: buscar, filtrar y ordenar sin tener que leer y parsear todo el archivo en memoria cada vez — ver [Índices](indexes.es.md).
- **Integridad**: reglas que la propia base fuerza, para que un dato nunca quede en un estado a medias o inválido — ver [ACID](acid.es.md).
...
- **Relacional**: el que ganó — tablas independientes de cómo se accede a ellas después, consultadas con SQL declarativo en vez de navegación manual entre registros. Ver [RDBMS](rdbms.es.md).
...
- **Semi-estructurado**: JSON/XML sin schema fijo — es lo que hoy cubren las bases de datos NoSQL de tipo documento. Ver [NoSQL](nosql.es.md).
...
- **Relacional (RDBMS)**: datos organizados en tablas con relaciones estrictas entre ellas, SQL como lenguaje de consulta — ver [RDBMS](rdbms.es.md).
- **NoSQL**: se aparta del modelo de tablas para priorizar escalabilidad horizontal o esquemas flexibles — ver [NoSQL](nosql.es.md).

---
Relacionado: [RDBMS](rdbms.es.md), [NoSQL](nosql.es.md).
